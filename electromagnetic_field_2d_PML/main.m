%% ========================================================================
% Code for 2D TE mode wave propagation with PML absorbing boundary
...
[Dz,Dx] = pml2d(c*ones(NZ,NX),nlayer,dh,dh,NX,NZ);
coef1 = (Dx*dt)/2;
coef2 = (Dz*dt)/2;
...
A21 = (1-b-coef2)./(1+b+coef2);
A22 = (dt./epsilon)./((1+b+coef2)*dh);
A31 = (1-coef2)./(1+coef2);
A32 = dt./((mux*dh).*(1+coef2)); 
A41 = (1-coef1)./(1+coef1);
A42 = -dt./((muz*dh).*(1+coef1)); 
...
slice = 5; % the number of time steps is saved as a snpshot
...
Hx = zeros(NZ,NX);    
for it = 1:nt       
    % Calculate the Eyx and Eyz field
    for j = 3:NZ-1
      for i = 3:NX-1
        Eyx(i,j) = A11(i,j)*Eyx(i,j) + A12(i,j)*((c1*(Hz(i,j)-Hz(i,j-1))+c2*(Hz(i,j+1)-Hz(i,j-2))));                           
        Eyz(i,j) = A21(i,j)*Eyz(i,j) + A22(i,j)*(c1*(Hx(i,j)-Hx(i-1,j))+c2*(Hx(i+1,j)-Hx(i-2,j)));
      end
    end
    ...
    for j = 2:NZ-2
      for i = 2:NX-2
        Hx(i,j) = A31(i,j)*Hx(i,j) + A32(i,j)*(c1*(Ey(i+1,j)-Ey(i,j))+c2*(Ey(i+2,j)-Ey(i-1,j)));
        Hz(i,j) = A41(i,j)*Hz(i,j) + A42(i,j)*(c1*(Ey(i,j+1)-Ey(i,j))+c2*(Ey(i,j+2)-Ey(i,j-1)));
      end
    end

    % Load source 
    Eyx(nlayer+sz,nlayer+sx(k)) = Eyx(nlayer+sz,nlayer+sx(k)) + src(it);
    ...
    % Show result
    if(k==ntrack) % plot the middle source reslut
